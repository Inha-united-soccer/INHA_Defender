...
{
    double length;            // x축 필드 길이
    double width;             // y축 전체 폭
    double penaltyDist;       // 패널티 스팟이 골라인으로부터 떨어진 거리
    double goalWidth;         // 골대 폭
    double circleRadius;      // 센터서클 반지름
    double penaltyAreaLength; // 패널티 지역의 길이 x
    double penaltyAreaWidth;  // 패널티 지역의 폭 y
    double goalAreaLength;    // 골키퍼 구역 길이
    double goalAreaWidth;     // 골키퍼 구역 폭
...
void BrainConfig::calcMapLines() {
    auto fd = fieldDimensions; // kid size의 맵을 가져옴  -> include/types.h에 정의돼잇음
    mapLines.clear(); 
